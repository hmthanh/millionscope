...
###### bar Qux [#]`,
  ...
  //   it("should merge headings from partial banner.tsx", async () => {
  //     const { result } = await compileMdx(
  //       `
...
describe("Link", () => {
  ...
  it("supports external .mdx links", async () => {
    const { result } = await compileMdx(`[link](https://example.com/file.mdx)`, {
      mdxOptions,
    });
    expect(result).toMatch(`<_components.a href="https://example.com/file.mdx">`);
  });
});
...
describe("Code block", () => {
  describe("Filename", () => {
    ...
    //       mdxOptions,
    //     });
    //     expect(result).toMatch('<_components.pre data-language="js" data-theme="default" filename="test.js">');
    //   });
    ...
    it("not highlight filename as substring", async () => {
      ...
      expect(result).not.toMatch('className="highlighted">{"foo"}</_components.span>');
  ...
  //     const { result } = await compileMdx('```js filename="test.js" {1}\n123\n```', {
  //       mdxOptions,
  //     });
