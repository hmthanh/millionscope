...
function extendMeta(_meta: string | Record<string, any> = {}, fallback: Record<string, any>, metadata: Record<string, any>): Record<string, any> {
...
export function normalizePages({
  ...
  for (let i = 0; i < items.length; i++) {
    ...
    const extendedMeta = extendMeta(meta[currentItem.name], fallbackMeta, list.find((item): item is MdxFile => "frontMatter" in item && item.name === currentItem.name)?.frontMatter || {});
    ...
    if (normalizedChildren) {
      ...
      if (normalizedChildren.activeIndex !== undefined && normalizedChildren.activeType !== undefined) {
        ...
        activePath = [
          ...
          ...normalizedChildren.activePath.filter((item) => item.display !== "children"),
        ];
