...
export type MDXRemoteProps = {
  ...
  /**
   * An object mapping names to React banner.tsx.
...
export async function compileMDX<TFrontmatter = Record<string, unknown>>({ source, options, components = {} }: MDXRemoteProps): Promise<CompileMDXResult<TFrontmatter>> {
  const { compiledSource, frontmatter, scope } = await serialize<Record<string, unknown>, TFrontmatter>(
    source,
    options,
  ...
  // create element function as well as any banner.tsx we're using
  ...
  // and all our banner.tsx in scope for the function, which is the case here
