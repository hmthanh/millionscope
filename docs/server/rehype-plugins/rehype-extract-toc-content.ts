...
import { TOC_HEADING_REGEX } from "@/server/constants";
import { createAstObject } from "@/server/utils";
import { VFile } from "vfile";

// todo rethink this
export const rehypeExtractTocContent: Plugin<[{ isRemoteContent?: boolean }], any> =
  ({ isRemoteContent }) =>
  (ast, file: VFile) => {
    const toc: any[] = [];
    ...
    const elements = file.data.toc.map((n) => {
      ...
      return createAstObject({
        value: result,
        id: node.openingElement.attributes.find((attr: JsxAttribute) => attr.name.name === "id").value.value,
