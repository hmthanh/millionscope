...
import type { MathJaxOptions } from "@/global/types";
      ...
      body: [
        {
          ...
          source: { type: "Literal", value: "nextra/banner.tsx" },
...
function wrapInMathJaxContext(children: RootContent[], { config, src }: NonNullable<MathJaxOptions>) {
      ...
      value: {
        ...
        data: {
          estree: {
            type: "Program",
            sourceType: "module",
            body: [{ type: "ExpressionStatement", expression: valueToEstree(config) }],
          },
        },
      },
    });
 ...
 */
function wrapInBraces(source: string, mathInline: boolean, options: NonNullable<MathJaxOptions>): string {
 ...
 */
export const rehypeBetterReactMathjax: Plugin<[Opts: MathJaxOptions, isRemoteContent: boolean], Root> =
