...
import type { NextraConfig, PageMapItem } from "@/global/types";
import PluggableList from "unified";
import { CHUNKS_DIR, CWD, DEFAULT_PROPERTY_PROPS, IMPORT_FRONTMATTER, MARKDOWN_EXTENSION_REGEX, META_REGEX } from "@/server/constants";
import { PAGES_DIR } from "@/server/file-system";
import { createAstExportConst, createAstObject, normalizePageRoute, pageTitleFromFilename, truthy } from "@/server/utils";
...
export async function collectFiles({ dir, route, imports = [], dynamicMetaImports = [], isFollowingSymlink }: CollectFilesOptions): Promise<{
  pageMap: PageMapItem[];
  imports: Import[];
  dynamicMetaImports: DynamicImport[];
