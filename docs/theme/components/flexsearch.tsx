...
import type { SearchData } from "@/global/types";
import { useRouter } from "@/client/hooks";
import type { ReactElement, ReactNode } from "react";
import { useCallback, useState } from "react";
import { DEFAULT_LOCALE } from "@/theme/constants";
...
const loadIndexesImpl = async (basePath: string, locale: string): Promise<void> => {
  const [searchData, Document] = await Promise.all([
    fetch(`${basePath}/_next/static/chunks/nextra-data-${locale}.json`).then((response) => response.json() as Promise<SearchData>),
