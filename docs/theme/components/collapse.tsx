...
export function Collapse({ children, className, isOpen, horizontal = false }: { children: ReactNode; className?: string; isOpen: boolean; horizontal?: boolean }): ReactElement {
  ...
  return (
    ...
    >
      <div ref={innerRef} className={cn("nx-transition-opacity nx-duration-500 nx-ease-in-out motion-reduce:nx-transition-none", isOpen ? "nx-opacity-100" : "nx-opacity-0", className)}>
        {children}
      </div>
    </div>
  );
