...
import { isFolder, isMeta, normalizeMeta } from "@/client/utils";
...
function getContext(functionName: string): {
  ...
  if (!__nextra_internal__) {
    throw new Error(`Nextra context not found. Make sure you are using "${functionName}" of "nextra/context" on an md/mdx page.`);
  }
  return {
